...
const SelectPopUp = ({ items, handleSelect, open, id, param1, param2 }) => {
  ...
  return (
    <Row className="justify-content-md-center align-items-center select-background">
      <Col md={4} className="select-container">
        ...
        />
        <div className="select-item-holder">
          {items.map((item, index) => {
            console.log({ item });
            if (item[id].toLowerCase().includes(querry.toLowerCase()))
              return (
                <div
                  ...
                  key={index}
                >
                  {param1 && <p className="select-item-one">+{item[param1]}</p>}
                  {param2 && <p>{item[param2]}</p>}
                  <p className="select-item-two">{item[id]}</p>
                </div>
              );
          })}
        </div>
