...
import AddStaffType from './AddStaffType/AddStaffType';
    ...
    return (
      <p
        ...
        onClick={() => {
          Swal.fire({
            title: 'Warning !!!',
            text: `Are you sure you want to delete ${row.staff_type_name}`,
            icon: 'warning',
            ...
            if (result.isConfirmed) {
              deleteStaffType(row.staff_type_code);
            }
          });
        }}
      >
  ...
  function editFormater(cell, row, index) {
    return (
      <p title="Edit">
        <svg
          onClick={async () => {
            // await selectInstitution({ institution_code: row.institution_code });
            // await handleChange(row.staff_type_name, 'staff_type_name');
            setShowAdd(row);
          }}
          style={{ cursor: 'pointer' }}
          width="20"
    ...
    <div>
      {showAdd && (
        <AddStaffType
          ...
          createStaffType={createStaffType}
          selectInstitution={selectInstitution}
          staffData={staffData}
          setShowAdd={setShowAdd}
        />
      )}
      <h3 className="staff-type-title">Staff Type</h3>
      <div className="staff-type-container">
        <div className="staffType-listing-title-container">
          <h4 className="staffType-listing-title m-4">Staff Type - Listing</h4>
          <button onClick={() => setShowAdd(true)} className="staff-type-btn m-4">
            Add New
          </button>
        </div>
        <Table
        ...
        />
        {staffTypes.length === 0 && !loading && (
          <div className="staff-type-lisitng-empty-table">
            <p className="staff-type-lisitng-empty-title">
