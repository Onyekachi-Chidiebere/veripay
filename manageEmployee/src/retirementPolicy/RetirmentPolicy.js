...
import AddRetirmentPolicy from './AddRetirmentPolicy';
...
const RetirementPolicy = () => {
    ...
    return (
      <p
        ...
        onClick={() => {
          Swal.fire({
            title: 'Warning !!!',
            text: `Are you sure you want to delete ${row.staff_type}`,
            icon: 'warning',
            ...
            if (result.isConfirmed) {
              deletePolicy(row.retirement_policy_code);
            }
          });
        }}
      >
  ...
  function date(cell, row) {
    ...
    return <p>{row.entered_on?row.entered_on.substring(0, 10):row.last_modified_on.substring(0,10)}</p>;
  }
  ...
  const columns = [
    ...
    {
      text: 'Max. Retirement Age',
      sortCaret: SortCaret,
      dataField: 'max_retirement_age',
      sort: true,
      headerStyle: (colum, colIndex) => {
        return { width: '150px', textAlign: 'start' };
      },
    ...
    {
      text: 'Active Service years',
      dataField: 'active_service_years',
      sort: true,
      sortCaret: SortCaret,
      headerStyle: (colum, colIndex) => {
        return { width: '150px', textAlign: 'start' };
  ...
  return (
    <div>
      {showAddPolicy && (
        <AddRetirmentPolicy
          close={() => setShowAddPolicy(false)}
          createRetirementPolicy={createRetirementPolicy}
          staffData={staffData}
          showAddPolicy={showAddPolicy}
          updateRetirementPolicy={updateRetirementPolicy}
          selectStaffType={selectStaffType}
          handleChange={handleChange}
          selectInstitution={selectInstitution}
          loading={loading}
        />
      )}
      <h3 className="staff-type-title">Retirement Policy</h3>

      <div className="staff-type-container">
        <div className="staffType-listing-title-container">
          <h4 className="staffType-listing-title m-4">
            Retirement Policy - Listing
          </h4>
          <button onClick={() => setShowAddPolicy(true)} className="staff-type-btn m-4">Setup Retirement Policy</button>
        </div>
        <Table
          columns={columns}
          rows={policies}
          ...
          // prev={{
          //   route: '/dashboard/setup/salary_structure/structure/category',
          //   title: 'Return to Structure Category',
          // }}
          // next={{
          //   route: '/dashboard/setup/salary_structure/grade-level',
          //   title: 'Proceed to  Grade level',
          // }}
...
export default RetirementPolicy;
