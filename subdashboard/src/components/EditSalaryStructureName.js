...
const EditSalaryStructureName = (props) => {
  const {data,setSuccess,setEdit,setData}=props;
  const {
  ...
  useEffect(() => {
    selectEditRegime(data.regime_code, data.category_code);
    handleChange(data.category_name, 'name');
    handleChange(data.structure_name, 'structure_name');
    handleChange(data.structure_abbreviation, 'structure_abbreviation');
  }, []);
  return (
    <div className="w-100 pt-4 edit-card">
      ...
      <Row className="justify-content-md-center align-items-center  h-100">
        ...
        <div className='d-flex justify-content-end align-items-center mx-3'>
          ...
          <p className='text-center'>Edit Salary Structure Name</p>
          <Select
          ...
          <button
            onClick={async()=>{
            let sent= await editStructureName()
            if(sent){
            setEdit(false);
              setData({
                  ...data,
                  structure_abbreviation:categoryData.structure_abbreviation,
                  structure_name:categoryData.structure_name,
                  category:categoryData.category,
                  regime:categoryData.regime,
              })
              setSuccess('Structure updated succesfully')
            }}}
...
export default EditSalaryStructureName;
