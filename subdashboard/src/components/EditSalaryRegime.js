...
import useAddSalary from '../hooks/useAddRegime';
import Select from './Select';
import TextInput from './TextInput';

const EditSalaryRegime = (props) => {
  ...
  useEffect(()=>{
    handleChange(months[Number(data.regime_month)-1], 'month');
    handleChange({year:data.regime_year}, 'year');
    handleChange(data.regime_name, 'name');
    handleChange(data.regime_code, 'regime_code');

  },[])

  return (
    <div className="w-100 edit-card">
         {error && (
        <SweetAlert
          danger
          title="Error"
      ...
      <Row className="justify-content-md-center align-items-center  h-100">
        <Col md={4} className='edit-container'>
          ...
          <p class="text-center">Edit Regime</p>
          <Select
            label="Year"
            text={regimeData.year.year}
            data={years}
            id="year"
           ...
           try {
            if(sent){
              ...
              setData({
                  ...
                  regime_name:regimeData.name,
              })
              setSuccess('Regime updated succesfully');
            }
           } catch (error) {
             console.log({error})
           }
...
export default EditSalaryRegime;
