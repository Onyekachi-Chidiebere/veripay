...
import EditGradeLevel from './EditSalaryGradeLevel';
import SortCaret from './table/SortCaret';
import Table from './table/Table';
import jsPDF from "jspdf";
import "jspdf-autotable";
import AddGradeLevel from './AddSalaryGradeLevel';

const SalaryGradeLevelList = () => {
  const [edit, setEdit]=useState(false)
  const [editIndex, setEditIndex]=useState(false)
  const [successEdit, setSuccessEdit]=useState(false)
  const [data, setData]=useState(false);
  const {confirmDel,setConfirmDel, getGradeLevelList,setGradeLevelList, gradeLevelList, loading, deleteGradeLevel, error, success, setSuccess } = useSalary();
  const [showAdd, setShowAdd] = useState(false);
  useEffect(()=>{
    if(successEdit){
      let newlist=[...gradeLevelList];
      newlist[editIndex]=data;
      setGradeLevelList(newlist);
    }
  }, [successEdit]);
  const handleUpdate = () => {
    getGradeLevelList(1)
  ...
  function status(cell, row) {
    if (row.active) {
      return (
        <span>
          <strong style={{ color: '#418E65' }}>Approved</strong>
        </span>
      );
    }
    return (
      <span>
        <strong style={{ color: '#CA6262' }}>Not Approved</strong>
      </span>
    );
  }
  ...
  function pensionC(cell, row) {
    if (row.pensionable) {
      return 'Pensionable';
    }
    return 'Not Pensionable';
  }
  function step(cell, row) {
    if (row.step_status) {
      return (
        <span>
          HAS STEP
  ...
  const exportPDF = () => {
    ...
    const title = "Grade Level List";
    const headers = [["Grade Level", "Regime", "Salary Structure","Status", "Pension Status", "Maximun Step", "Approval Status", "Modified By", "Modified On"]];

    const data = gradeLevelList.map(structure => [
      structure.grade_name,
      structure.regime,
      structure.salary_structure,
      structure.step_status ? 'HAS STEP' : 'NO STEP',
      structure.pensionable?"Pensionable": 'Not Pensionable',
      structure.maximum_step,
      structure.active?'Approved': 'Not Approved',
      structure.entered_by,
      structure.date.substring(0,10),
      '...',
      '...'
    ]);
  ...
  return (
    <div className="w-100 dashboard-main-card">
       {confirmDel&&<DeleteDialog name={confirmDel.grade_name} _delete={()=>{deleteGradeLevel(confirmDel.grade_level_code)}} closeDialog={()=>setConfirmDel(false)}/>}
      {edit&&<EditGradeLevel setData={setData} data={edit}  setEdit={setEdit} setSuccess={setSuccessEdit}/>}
      {showAdd && <AddGradeLevel close={() => setShowAdd(false)} handleUpdate={handleUpdate} />}
    ...
    <div className="w-100 pt-4 dashboard-card">
     {successEdit && (
      ...
      <div className="salary-regime-menu d-flex justify-content-between mt-4 mx-4 align-items-center">
        <span className="salary-regime-title fs-6 mx-3 mb-3">
          View Grade Level (Approved Listing)
        </span>
          <NavLink to="" onClick={(e) => { e.preventDefault(); setShowAdd(true); }} className="btn dashboard-add-btn mx-3 mb-3">
          {/* add icon */}
          <svg
            width="24"
            height="24"
          ...
          <span className='fs-7'>Add Grade Level</span>
        </NavLink>
      </div>
      <div>
        ...
        <Table columns={columns} rows={gradeLevelList} csvTitle={'gradelevel.csv'}  search={true} exportPdf={exportPDF}  prev={{ title:'Return to Salary Structure', route:'/dashboard/setup/salary_structure/structure'}} next={{ title:'Proceed to  Basic Scale', route:'/dashboard/setup/salary_structure/basic-scale'}}/>
        {gradeLevelList.length === 0 && !loading && (
          <DashboardEmpty
            title="Add Grade level"
            text="No available Salary Grade"
...
export default SalaryGradeLevelList;
