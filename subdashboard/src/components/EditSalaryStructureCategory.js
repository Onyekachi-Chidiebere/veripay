...
const EditSalaryStructureCategory = (props) => {
  ...
  useEffect(() => {
    ...
    selectEditRegime(data.regime_code);
    handleChange(data.category_name, 'name');
    handleChange(data.category_code, 'category_code');
  },[])
  return (
        ...
        <Col md={4} className='edit-container'>
          ...
          <div className="mt-4 mx-3">
            <button onClick={async () => {
              let sent = await handleEditCategorySubmit();
              if (sent) {
                setEdit(false);
                setData({
                  ...data,
              ...
              setSuccess('Category updated succesfully')
              }
...
export default EditSalaryStructureCategory;
