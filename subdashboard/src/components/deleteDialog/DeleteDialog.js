...
const DeleteDialog=(props)=>{
    ...
    const {name,_delete,closeDialog}=props;
    return <div className='delete-dialog-background'>
        <div className='delete-container'>
          <div className='delete-close-icon'>
          <svg
            onClick={()=>closeDialog()}
          ...
          >
            ...
            <p>Please enter the name below to confirm</p>
            <div className='delete-input-holder'>
            <input value={val} onChange={({target})=>setVal(target.value)}/>
            <button onClick={_delete} disabled={name!==val}>Delete</button>
            </div>
        </div>
    </div>
};
